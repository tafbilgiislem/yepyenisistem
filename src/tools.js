...
window.safeColor = function(c) { 
    if(!c) return "#ffffff"; 
    if(c.length === 4 && c.startsWith('#')) return "#" + c[1] + c[1] + c[2] + c[2] + c[3] + c[3]; 
    return c.startsWith('#') && c.length === 7 ? c : "#ffffff"; 
};
...
window.addNewText = function() {
    ...
    t.setAttribute("x", center.cx); 
    t.setAttribute("y", center.cy); 
    t.setAttribute("text-anchor", "middle"); 
    t.setAttribute("dominant-baseline", "central"); 
    window.setD(t, 'base-font-size', "80"); 
    window.setD(t, 'raw-text', "YENİ METİN"); 
    window.setD(t, 'solid-color', "#ffffff"); 
    ...
    if(window.updateUI) window.updateUI(t); 
    if(window.renderEditor) window.renderEditor();
};

// 2. Nesne Ekleme: Şekil / Kutu
window.addShape = function() {
    const svg = document.querySelector('#canvas-inner svg'); if(!svg) return; 
    const center = window.getCanvasCenter();
    const rect = document.createElementNS("http://www.w3.org/2000/svg", "rect"); 
    rect.id = "shp_" + Date.now(); 
    rect.setAttribute("class", "duzenlenebilir"); 
    rect.setAttribute("x", center.cx - 100); 
    rect.setAttribute("y", center.cy - 100); 
    rect.setAttribute("width", 200); 
    rect.setAttribute("height", 200); 
    window.setD(rect, 'solid-color', "#10b981"); 
    rect.setAttribute("fill", "#10b981"); 
    window.setD(rect, 'mask-shape', "none");
    svg.appendChild(rect); 
    
    window.selectedEl = rect; 
    if(window.saveState) window.saveState(); 
    if(window.setupLayers) window.setupLayers(); 
    if(window.updateUI) window.updateUI(rect); 
    if(window.renderEditor) window.renderEditor();
};

// 3. Nesne Ekleme: Video / YouTube
window.addVideo = function() {
    ...
    fo.setAttribute("x", dim.w/2 - 400); fo.setAttribute("y", dim.h/2 - 225);
    fo.setAttribute("width", 800); fo.setAttribute("height", 450);
    window.setD(fo, 'video-url', 'https://www.w3schools.com/html/mov_bbb.mp4');
    window.setD(fo, 'video-muted', 'true'); window.setD(fo, 'video-loop', 'true');
    window.setD(fo, 'mask-shape', 'none'); window.setD(fo, 'rx', '0');
    svg.appendChild(fo); 
    
    window.updateVideoDisplay(fo);
    window.selectedEl = fo; 
    if(window.saveState) window.saveState(); 
    if(window.setupLayers) window.setupLayers(); 
    if(window.updateUI) window.updateUI(fo); 
    if(window.renderEditor) window.renderEditor();
};
...
window.updateVideoDisplay = function(el) {
    ...
    if(window.applyShapeMask) window.applyShapeMask(el); 
};

// 4. Nesne Ekleme: RSS Haber Bandı
window.addRssBand = function() {
    const svg = document.querySelector('#canvas-inner svg'); if(!svg) return; 
    const dim = window.getSvgDim();
    const fo = document.createElementNS("http://www.w3.org/2000/svg", "foreignObject");
    fo.id = "rss_" + Date.now(); fo.setAttribute("class", "duzenlenebilir rss-band");
    fo.setAttribute("x", 0); fo.setAttribute("y", dim.h - 80);
    fo.setAttribute("width", dim.w); fo.setAttribute("height", 80);
    window.setD(fo, 'rss-url', 'https://www.cnnturk.com/feed/rss/all/news');
    window.setD(fo, 'rss-speed', '35'); window.setD(fo, 'solid-color', '#dc2626');
    window.setD(fo, 'text-color', '#ffffff'); window.setD(fo, 'base-font-size', '30');
    svg.appendChild(fo); 
    
    window.updateRssDisplay(fo);
    window.selectedEl = fo; 
    if(window.saveState) window.saveState(); 
    if(window.setupLayers) window.setupLayers(); 
    if(window.updateUI) window.updateUI(fo); 
...
// 5. Nesne Ekleme: Analog Hava Durumu Saati
window.addWeather = function() {
    const svg = document.querySelector('#canvas-inner svg'); if(!svg) return; 
    const center = window.getCanvasCenter();
    const fo = document.createElementNS("http://www.w3.org/2000/svg", "foreignObject");
    fo.id = "wth_" + Date.now(); fo.setAttribute("class", "duzenlenebilir weather-widget");
    fo.setAttribute("x", center.cx - 250); fo.setAttribute("y", center.cy - 250); 
    fo.setAttribute("width", 500); fo.setAttribute("height", 500);
    window.setD(fo, 'city', 'Istanbul'); window.setD(fo, 'theme', 'dark'); 
    window.setD(fo, 'mask-shape', 'circle'); window.setD(fo, 'rx', '0'); 
    fo.setAttribute('font-family', 'sans-serif');
    
    svg.appendChild(fo); 
    window.updateWeatherDisplay(fo);
    window.selectedEl = fo; 
    if(window.saveState) window.saveState(); 
    if(window.setupLayers) window.setupLayers(); 
    if(window.updateUI) window.updateUI(fo); 
...
window.addCurrency = function() {
    ...
    window.setD(fo, 'currencies', 'USD,EUR,GBP'); window.setD(fo, 'solid-color', '#000000'); 
    window.setD(fo, 'bg-opacity', '0.6'); window.setD(fo, 'text-color', '#ffffff'); 
    window.setD(fo, 'mask-shape', 'none'); window.setD(fo, 'rx', '15'); fo.setAttribute('font-family', 'sans-serif');
    
    svg.appendChild(fo); 
    window.updateCurrencyDisplay(fo);
    window.selectedEl = fo; 
    if(window.saveState) window.saveState(); 
    if(window.setupLayers) window.setupLayers(); 
    if(window.updateUI) window.updateUI(fo); 
    ...
    if (file.type === 'image/svg+xml' || file.name.toLowerCase().endsWith('.svg')) {
        ...
        reader.onload = (ev) => {
            try {
                ...
                const center = window.getCanvasCenter();
                importedSvg.id = "svg_import_" + Date.now(); 
                importedSvg.setAttribute("class", "duzenlenebilir");
                importedSvg.style.position = ""; importedSvg.style.left = ""; importedSvg.style.top = "";
                
                let w = parseFloat(importedSvg.getAttribute('width')) || 400; 
                let h = parseFloat(importedSvg.getAttribute('height')) || 400;
                
                if(importedSvg.hasAttribute('viewBox') && (!importedSvg.hasAttribute('width') || !importedSvg.hasAttribute('height'))) {
                    const vb = importedSvg.getAttribute('viewBox').split(/\s+|,/); 
                    if(vb.length >= 4) { w = parseFloat(vb[2]); h = parseFloat(vb[3]); importedSvg.setAttribute('width', w); importedSvg.setAttribute('height', h); }
                }
                
                importedSvg.setAttribute("x", center.cx - (w / 2)); 
                importedSvg.setAttribute("y", center.cy - (h / 2));
                mainSvg.appendChild(importedSvg); 
                
                window.selectedEl = importedSvg; 
                if(window.saveState) window.saveState(); 
                if(window.setupLayers) window.setupLayers(); 
                if(window.updateUI) window.updateUI(importedSvg); 
                if(window.renderEditor) window.renderEditor(); 
                window.showToast("SVG Eklendi", "success");
        ...
        reader.onload = (ev) => {
            ...
            img.setAttribute("class", "duzenlenebilir"); 
            img.setAttribute("x", center.cx - 150); 
            img.setAttribute("y", center.cy - 150); 
            img.setAttribute("width", 300); 
            img.setAttribute("height", 300); 
            window.setD(img, "rx", "0"); 
            window.setD(img, "smoothing", "0.5"); 
            window.setD(img, "mask-shape", "none"); 
            img.setAttribute("href", ev.target.result);
            svg.appendChild(img); 
            
            window.selectedEl = img; 
            if(window.saveState) window.saveState(); 
            if(window.setupLayers) window.setupLayers(); 
            if(window.updateUI) window.updateUI(img); 
            if(window.renderEditor) window.renderEditor();
        }; 
...
// Sürükle ve Bırak Event Listener'ları
setTimeout(() => {
...
}, 500); // DOM'un yüklenmesini ufak bir gecikme ile garantile