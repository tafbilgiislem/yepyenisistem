...
// =========================================================================
// 🚀 UZAKTAN KUMANDA SİNYALLERİNİ DİNLEME (F5 & BUL)
// =========================================================================

// Cihazın kendi ID'sini bul (Sisteminde cihaz ID'si hangi değişkendeyse onu kullanır)
const aktifCihazId = window.cihazId || localStorage.getItem('cihaz_id') || localStorage.getItem('tv_id') || "BİLİNMEYEN_CİHAZ";

if (aktifCihazId && aktifCihazId !== "BİLİNMEYEN_CİHAZ") {
    let sonKomutZamani = Date.now(); // TV ilk açıldığında eski komutları (tarihi geçmiş) yoksayması için

    import("firebase/database").then((firebaseDB) => {
        const cihazRef = firebaseDB.ref(db, `sahne/cihazlar/${aktifCihazId}`);
        
        firebaseDB.onValue(cihazRef, (snapshot) => {
            if (snapshot.exists()) {
                const data = snapshot.val();
                
                // Yeni bir komut gelmiş mi ve zamanı bizim TV'nin açılışından daha mı yeni?
                if (data.command && data.commandTime && data.commandTime > sonKomutZamani) {
                    sonKomutZamani = data.commandTime; // Komutu işledik, zamanı güncelle ki bir daha yapmasın
                    
                    // 🔄 F5 (YENİLE) KOMUTU GELDİYSE
                    if (data.command === 'refresh') {
                        console.log("Kumandadan F5 (Yenile) komutu geldi!");
                        // Ekranı çok ani patlatmamak için 1 saniye bekleyip zorla yeniler
                        setTimeout(() => {
                            window.location.reload(true);
                        }, 1000);
                    } 
                    
                    // 📍 BUL (PİNG) KOMUTU GELDİYSE
                    else if (data.command === 'ping') {
                        console.log("Kumandadan Bul (Ping) komutu geldi!");
                        gorselPingGoster(data.name || aktifCihazId);
                    }
                }
            }
        });
...
// 📍 TV EKRANINDA KOCAMAN "BURADAYIM" YAZISI ÇIKARTAN FONKSİYON
function gorselPingGoster(cihazIsmi) {
    // Ekranda eski bir ping uyarısı varsa temizle
    const eskiUyari = document.getElementById('ping-overlay-tv');
    if (eskiUyari) eskiUyari.remove();

    // Dev bir kırmızı uyarı ekranı oluştur
    const overlay = document.createElement('div');
    overlay.id = 'ping-overlay-tv';
    overlay.style.cssText = 'position:fixed; top:0; left:0; width:100vw; height:100vh; background:rgba(220, 38, 38, 0.95); z-index:9999999; display:flex; flex-direction:column; justify-content:center; align-items:center; color:white; font-family:sans-serif; animation: pingPulse 1s infinite alternate;';
    
    overlay.innerHTML = `
        <style>@keyframes pingPulse { 0% { transform: scale(0.95); } 100% { transform: scale(1.05); } }</style>
        <div style="font-size: 150px; margin-bottom: 20px;">📍</div>
        <h1 style="font-size: 100px; font-weight: 900; margin: 0; text-shadow: 0 10px 30px rgba(0,0,0,0.5); letter-spacing: 5px;">BURADAYIM!</h1>
        <h2 style="font-size: 50px; margin-top: 30px; font-weight: bold; color: #fca5a5; background: rgba(0,0,0,0.4); padding: 15px 40px; border-radius: 20px;">${cihazIsmi}</h2>
    `;

    document.body.appendChild(overlay);

    // 5 saniye boyunca ekranda bağırdıktan sonra usulca kaybolsun
    setTimeout(() => {
