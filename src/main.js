...
// =========================================================================
// 🚀 AKTİF CİHAZ YÖNETİM FONKSİYONLARI (İSİM, GRUP, F5, BUL)
// =========================================================================

window.renameDevicePrompt = function(deviceId, currentName) {
    const newName = prompt('Cihaz için yeni bir isim girin:', currentName);
    if (newName && newName.trim() !== '' && newName !== currentName) {
        // Firebase kütüphanesini güvenli şekilde çağırıp veriyi güncelliyoruz
        import("firebase/database").then((firebaseDB) => {
            firebaseDB.update(firebaseDB.ref(db, `sahne/cihazlar/${deviceId}`), { 
...
window.changeDeviceGroupPrompt = function(deviceId, currentGroup) {
    const newGroup = prompt('Cihazın yeni grubunu girin (Örn: LOBİ, VİTRİN, MERKEZ):', currentGroup);
    if (newGroup && newGroup.trim() !== '' && newGroup !== currentGroup) {
        import("firebase/database").then((firebaseDB) => {
            firebaseDB.update(firebaseDB.ref(db, `sahne/cihazlar/${deviceId}`), { 
                group: newGroup.trim().toUpperCase() 
            });
        });
    }
};

window.sendCommand = function(deviceId, cmd) {
    import("firebase/database").then((firebaseDB) => {
        firebaseDB.update(firebaseDB.ref(db, `sahne/cihazlar/${deviceId}`), {
            command: cmd,
            commandTime: Date.now() // TV'nin her basılışta yeni bir komut geldiğini anlaması için
        }).then(() => {
            if (cmd === 'ping') alert('📍 Bul (Ping) sinyali gönderildi!');
            if (cmd === 'refresh') alert('🔄 TV Ekranı yenilenmeye zorlanıyor (F5)...');
        });
    });
};