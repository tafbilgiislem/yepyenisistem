window.applyRawCode = function(id) {
    try { 
        let p = new DOMParser(); let code = document.getElementById('raw-svg-code').value.trim(); let n; 
        if (!code.toLowerCase().startsWith('<svg')) { let d = p.parseFromString('<svg xmlns="http://www.w3.org/2000/svg">' + code + '</svg>', 'image/svg+xml'); n = d.documentElement.firstElementChild; } 
        else { let d = p.parseFromString(code, 'image/svg+xml'); n = d.documentElement; } 
        if(!n) throw new Error('Geçersiz SVG'); 
        let o = document.getElementById(id); o.replaceWith(n); window.selectedEl = n; 
        ...
        if(window.saveState) window.saveState(); if(window.setupLayers) window.setupLayers(); if(window.updateUI) window.updateUI(window.selectedEl); if(window.renderEditor) window.renderEditor(); if(window.showToast) window.showToast("Uygulandı!"); 
    } catch(err) { if(window.showToast) window.showToast('Hatalı Kod.', 'error'); }
};
...
window.initEngine = function(svg) {
    ...
    if(wrapper) {
        wrapper.onpointerdown = (e) => {
            if (window.isSpacePressed) { window.isPanning = true; window.panStart = {x: e.clientX, y: e.clientY}; window.panOffsetStart = {x: window.panX, y: window.panY}; document.getElementById('svg-wrapper')?.classList.add('panning'); return; }
            const p = getCoords(e); const target = e.target; window.isModified = false;
            
            if (window.isDrawingMode) {
                window.isDrawing = true; window.currentPath = document.createElementNS("http://www.w3.org/2000/svg", "path"); window.currentPath.id = "pth_" + Date.now(); window.currentPath.setAttribute("class", "duzenlenebilir"); window.currentPath.setAttribute("fill", "none"); window.currentPath.setAttribute("stroke", "#10b981"); window.currentPath.setAttribute("stroke-width", "5"); window.currentPath.setAttribute("stroke-linecap", "round"); window.currentPath.setAttribute("stroke-linejoin", "round"); window.pathData = `M ${p.x} ${p.y}`; window.currentPath.setAttribute("d", window.pathData);
                const guidesGroup = document.getElementById('guides-group'); if(guidesGroup) svg.insertBefore(window.currentPath, guidesGroup); else svg.appendChild(window.currentPath);
                try { wrapper.setPointerCapture(e.pointerId); } catch(err){} return;
            }
            
            if (target.classList.contains('guide-line')) { window.isDraggingGuide = true; window.currentGuide = target; try { wrapper.setPointerCapture(e.pointerId); } catch(err){} e.preventDefault(); e.stopPropagation(); return; }
            ...
            if (target.classList.contains('handle')) {
                ...
                window.startP = p; window.startData = { x: parseFloat(el.getAttribute("x")) || 0, y: parseFloat(el.getAttribute("y")) || 0, w: tempW, h: tempH, rx: parseFloat(window.getD(el, 'rx')) || 0, baseFontSize: parseFloat(window.getD(el, 'base-font-size')) || 60 };
            ...
            if (el) {
                ...
                window.isDraggingElement = true; window.offset.x = p.x - (parseFloat(window.selectedEl.getAttribute("x")) || 0); window.offset.y = p.y - (parseFloat(window.selectedEl.getAttribute("y")) || 0); 
                if(window.updateUI) window.updateUI(window.selectedEl); if(window.renderEditor) window.renderEditor();
                if(window.getD(window.selectedEl, 'locked') !== "true") { try { wrapper.setPointerCapture(e.pointerId); } catch(err){} }
            } else { if(!e.target.closest('#sidebar')) { window.selectedEl = null; if(ctrl) ctrl.innerHTML = ""; if(window.renderEditor) window.renderEditor(); } }
        };

        wrapper.onpointermove = (e) => {
            if (window.isPanning) { window.panX = window.panOffsetStart.x + ((e.clientX - window.panStart.x) / window.currentZoom); window.panY = window.panOffsetStart.y + ((e.clientY - window.panStart.y) / window.currentZoom); if(window.applyZoom) window.applyZoom(); if(window.syncRulerTransform) window.syncRulerTransform(); return; }
            const p = getCoords(e);
            ...
            if (window.isDraggingGuide && window.currentGuide) { if(window.getD(window.currentGuide, 'type') === 'h') { window.currentGuide.setAttribute('y1', p.y); window.currentGuide.setAttribute('y2', p.y); } else { window.currentGuide.setAttribute('x1', p.x); window.currentGuide.setAttribute('x2', p.x); } return; }
            ...
            if (window.resizingEl) {
                ...
                if (window.activeHandle === 'br') { w += dx; h += dy; } else if (window.activeHandle === 'bl') { x += dx; w -= dx; h += dy; } else if (window.activeHandle === 'tr') { y += dy; w += dx; h -= dy; } else if (window.activeHandle === 'tl') { x += dx; y += dy; w -= dx; h -= dy; }
                if (window.resizingEl.tagName === "text") { 
                    if(w > 10) { window.resizingEl.setAttribute('textLength', w); window.resizingEl.setAttribute('lengthAdjust', 'spacingAndGlyphs'); const tp = window.resizingEl.querySelector('textPath'); if (tp) { tp.setAttribute('textLength', w); tp.setAttribute('lengthAdjust', 'spacingAndGlyphs'); } }
                    if(h > 5) { window.setD(window.resizingEl, 'base-font-size', h); } 
                    if(window.activeHandle.includes('l')) window.resizingEl.setAttribute('x', x); if(window.activeHandle.includes('t')) window.resizingEl.setAttribute('y', y); 
                    ...
                    if (w > 10 && h > 10) { window.resizingEl.setAttribute("x", x); window.resizingEl.setAttribute("y", y); window.resizingEl.setAttribute("width", w); window.resizingEl.setAttribute("height", h); } 
                } 
                if(window.updateUI) window.updateUI(window.resizingEl); 
                if(window.resizingEl.tagName === 'rect' || window.resizingEl.tagName === 'image' || window.resizingEl.classList.contains('video-obj') || window.resizingEl.classList.contains('weather-widget') || window.resizingEl.classList.contains('currency-widget')) { document.querySelectorAll(`text[data-bound-rect="${window.resizingEl.id}"]`).forEach(txt => { if(window.autoFitText) window.autoFitText(txt); }); }
                ...
                window.setD(window.rotatingEl, 'angle', angle); if(window.applyTransforms) window.applyTransforms(window.rotatingEl); if(window.updateUI) window.updateUI(window.rotatingEl);
                ...
                window.isModified = true; let delta = Math.max(dx, dy); let newRx = Math.max(0, Math.min(window.startData.w/2, window.startData.h/2, window.startData.rx + delta));
                window.setD(window.radiusingEl, 'rx', newRx); if(window.applyShapeMask) window.applyShapeMask(window.radiusingEl); if(window.updateUI) window.updateUI(window.radiusingEl);
                let badge = document.getElementById('val-rx-' + window.radiusingEl.id); if(badge) badge.innerText = Math.round(newRx) + 'px';
                ...
                window.isModified = true; let newX = p.x - window.offset.x; let newY = p.y - window.offset.y;
                ...
                if((window.selectedEl.tagName === 'image' || window.selectedEl.tagName === 'rect' || window.selectedEl.classList.contains('video-obj') || window.selectedEl.classList.contains('weather-widget') || window.selectedEl.classList.contains('currency-widget')) && window.getD(window.selectedEl, 'mask-shape') && window.getD(window.selectedEl, 'mask-shape') !== "none") if(window.applyShapeMask) window.applyShapeMask(window.selectedEl); 
                if(window.selectedEl.tagName === 'text' && window.getD(window.selectedEl, 'curve')) if(window.applyTextCurve) window.applyTextCurve(window.selectedEl); 
                if(window.updateUI) window.updateUI(window.selectedEl, snapX, snapY);
                if(window.selectedEl.tagName === 'rect' || window.selectedEl.tagName === 'image' || window.selectedEl.classList.contains('video-obj') || window.selectedEl.classList.contains('weather-widget') || window.selectedEl.classList.contains('currency-widget')) { document.querySelectorAll(`text[data-bound-rect="${window.selectedEl.id}"]`).forEach(txt => { if(window.autoFitText) window.autoFitText(txt); }); }
            }
        };

        const release = (e) => { 
            try { wrapper.releasePointerCapture(e.pointerId); } catch(err){} window.isDraggingElement = false; 
            if (window.isPanning) { window.isPanning = false; document.getElementById('svg-wrapper')?.classList.remove('panning'); return; }
            if (window.isDrawing) { window.isDrawing = false; window.selectedEl = window.currentPath; window.currentPath = null; window.isDrawingMode = false; document.getElementById('svg-wrapper')?.classList.remove('draw-mode'); if(window.saveState) window.saveState(); if(window.renderEditor) window.renderEditor(); if(window.updateUI) window.updateUI(window.selectedEl); return; }
            if (window.isDraggingGuide) { window.isDraggingGuide = false; window.currentGuide = null; if(window.saveState) window.saveState(); return; }
            if (window.isModified) { if(window.saveState) window.saveState(); if(window.selectedEl && window.renderProperties) window.renderProperties(); }
            window.resizingEl = null; window.rotatingEl = null; window.radiusingEl = null; window.activeHandle = null; window.isModified = false; document.querySelectorAll('.snap-line').forEach(l=>l.remove()); 
        };
...
window.updateUI = function(el, snapX=null, snapY=null) {
    ...
    if (el.tagName === 'rect' || el.tagName === 'image' || el.classList.contains('video-obj') || el.classList.contains('weather-widget') || el.classList.contains('currency-widget')) {
        const currentRx = parseFloat(window.getD(el, 'rx')) || 0; const offsetDist = Math.max(15, currentRx);
        const radH = document.createElementNS("http://www.w3.org/2000/svg", "circle"); radH.setAttribute("cx", b.x + offsetDist); radH.setAttribute("cy", b.y + offsetDist); radH.setAttribute("r", s/2.5); radH.setAttribute("class", "handle handle-radius"); radH.dataset.id = el.id; g.appendChild(radH);
    }
};
